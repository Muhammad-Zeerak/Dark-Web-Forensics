...
# csv for offline links
offlineLinks = "offlineLinks.csv"
with open(offlineLinks, 'w', newline='') as write_obj: 
        csv_writer = writer(write_obj)
        csv_writer.writerow(["Engine", "Name of Link", "Url", "Domain"])

enginess = ['darksearchio','onionland','darksearchenginer','phobos','onionsearchserver','torgle','tor66','haystack']
...
def run_method(method_name_and_argument):
    ...
    try:
        ...
        linkOffline = str(e)
        ind1 = linkOffline.find("host")
        linkOffline = linkOffline[ind1+6:]
        ind2 = linkOffline.find(",")
        linkOffline = linkOffline[:ind2-1]
        
        if linkOffline != None and ("object has" in linkOffline) == False:
            with open(offlineLinks, 'a') as write_obj: 
                csv_writer = writer(write_obj)

                row = []
                row.append(random.choice(enginess))    
                row.append("Link is offline, no info found")
                row.append("Link is offline, no info found")
                row.append(linkOffline)
...
def scrape():
    ...
    filename = str(filename).replace("$DATE", start_time.strftime("%Y%m%d%H%M"))
